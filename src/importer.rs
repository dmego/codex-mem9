...
use crate::{config::RuntimeConfig, mem9::{Mem9Client, StorePayload}, redact::sanitize_mem9_content, state::SyncState};
...
pub async fn sync_once(config: &RuntimeConfig) -> Result<SyncStats> {
    ...
    for item in items {
        ...
        let store_result = client.store(&StorePayload {
...
fn collect_from_file(path: &Path) -> Result<Vec<ImportItem>> {
    ...
    match file_name {
        "MEMORY.md" => {
            items.extend(collect_tagged_bullets(&markdown, "### learnings", "Learning", &["codex-memory", "learning"], &source));
        }
        "memory_summary.md" => {
            items.extend(collect_tagged_bullets(&markdown, "## User preferences", "User preference", &["codex-memory", "user-preference"], &source));
            items.extend(collect_tagged_bullets(&markdown, "## General Tips", "General tip", &["codex-memory", "general-tip"], &source));
        }
        _ => {
            items.extend(collect_tagged_bullets(&markdown, "### learnings", "Learning", &["codex-memory", "rollout-learning"], &source));
        }
    }
...
fn collect_tagged_bullets(markdown: &str, heading: &str, label: &str, tags: &[&str], source: &str) -> Vec<ImportItem> {
    let mut items = Vec::new();
...
fn build_source(path: &Path) -> String {
    ...
    format!("codex-memory:{}", parts.into_iter().collect::<Vec<_>>().join("/"))
}
...
mod tests {
    ...
    fn collects_high_signal_entries() {
        ...
        fs::write(root.join("MEMORY.md"), "### learnings\n- Re-check `~/.codex/sessions/x.jsonl`\n").unwrap();
        fs::write(root.join("memory_summary.md"), "## User preferences\n- Prefer concise Chinese output\n## General Tips\n- Check localhost:8081 first\n").unwrap();
        fs::write(root.join("rollout_summaries").join("one.md"), "### learnings\n- The service listens on 192.168.64.1:5000\n").unwrap();

        let items = collect_import_entries(&root).unwrap();
        assert_eq!(items.len(), 4);
        assert!(items.iter().any(|item| item.content.contains("Codex session rollout JSONL file")));
        assert!(items.iter().any(|item| item.content.contains("loopback address")) || items.iter().any(|item| item.content.contains("related address")));
        assert!(items.iter().any(|item| item.content.contains("private network address (port 5000)")));
    }
    ...
    async fn sync_once_continues_when_store_request_fails() {
        ...
        fs::write(memories_dir.join("MEMORY.md"), "### learnings\n- One durable fact\n").unwrap();
